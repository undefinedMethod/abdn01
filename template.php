...
// Add Zen Tabs styles
if (theme_get_setting('abdn01_tabs')) {
  drupal_add_css( drupal_get_path('theme', 'abdn01') .'/css/tabs.css');
}

function abdn01_preprocess_html(&$vars) {
  global $user, $language;

  // Add role name classes (to allow css based show for admin/hidden from user)
  foreach ($user->roles as $role){
    $vars['classes_array'][] = 'role-' . abdn01_id_safe($role);
  }
  ...
  if (!$vars['is_front']) {
    ...
    $vars['classes_array'][] = abdn01_id_safe('page-'. $path);
    $vars['classes_array'][] = abdn01_id_safe('section-'. $section);
  ...
  // Add IE classes.
  if (theme_get_setting('abdn01_ie_enabled')) {
    $abdn01_ie_enabled_versions = theme_get_setting('abdn01_ie_enabled_versions');
    if (in_array('ie8', $abdn01_ie_enabled_versions, TRUE)) {
...
function abdn01_preprocess_page(&$vars, $hook) {
...
function abdn01_preprocess_node(&$vars) {
  ...
  // Merge first/last class (from abdn01_preprocess_page) into classes array of current node object.
  $node = $vars['node'];
...
function abdn01_preprocess_block(&$vars, $hook) {
  // Add a striping class.
 ...
 */
function abdn01_breadcrumb($variables) {
  $breadcrumb = $variables['breadcrumb'];
  // Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('abdn01_breadcrumb');
  if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {

    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('abdn01_breadcrumb_home');
    ...
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = theme_get_setting('abdn01_breadcrumb_separator');
      $trailing_separator = $title = '';
      if (theme_get_setting('abdn01_breadcrumb_title')) {
        $item = menu_get_item();
      ...
      elseif (theme_get_setting('abdn01_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }
 ...
 */
function abdn01_id_safe($string) {
 ...
 *
 */
function abdn01_menu_link(array $variables) {
  ...
  $element['#attributes']['class'][] = abdn01_id_safe($element['#title']);
 ...
 */
function abdn01_preprocess_menu_local_task(&$variables) {
 ...
 */
function abdn01_menu_local_tasks(&$variables) {
  $output = '';
